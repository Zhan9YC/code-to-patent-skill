...
若用户提供 `.docx` / `.md` / `.dotx` 等交底书模板，或明确说“模板在某目录/按该模板写”，则**用户模板优先于本节默认结构**：

1. **必须先读取模板**：`.docx` 须先转 Markdown 扫描语义，同时用能保留 Word 结构的方式检查表格、段落、页眉页脚、图片占位、字段名和章节顺序。
2. **按模板填充**：正文须落入模板已有字段、表格和章节，不得强行改成“1.1/1.2/3.4.1”等默认结构；默认结构只能作为内容完整性参考。
3. **Word 风格保持**：交付 `.docx` 时应优先以用户原 `.docx` 模板为基底写入，保留表格结构、标题样式、字体、页边距和顺序；不得只用 Markdown 转 Word 生成一个风格不一致的新文档。
4. **图示适配模板**：模板要求插图时，Word 中应插入 PNG/图片对象或模板可接受的图示，不得把未渲染的 mermaid 代码块当作图示交付。
5. **同名文本备份**：仍须输出同名 `.md`，但该 Markdown 是文本备份，不得反过来破坏 Word 模板交付优先级。
...
**查新呈现边界（硬性）**：查新材料是“现有技术依据”，不是正文主角。正文中不得出现搜索过程、工具名、降级策略、检索失败说明、网页搜索痕迹或大段文献堆砌。默认正文只写：最接近通常做法、该做法的因果缺点、本案区别。若用户模板含“检索结果/对比文件”附表，则把精简后的现有技术条目放入附表；若无附表，可在 1.1 用 2-4 条代表性技术方向概括，不得把中间搜索结果流水账写入正文。
...
若用户提供 Word 模板且模板风格要求图片/Visio/可编辑图，图示交付以模板风格为准：可以使用生成的 PNG、表格图、或可编辑 Office 图形；不得在 Word 正文中残留 mermaid 代码块。若图示渲染失败，应修复、换用可插入图片，或在交付前明确说明限制，不得默默交付不符合模板的图示。
...
当 Step 2 的主要材料是代码文件、算法脚本、数据处理流程或命令行工具时，交底书的“具体实施方式”必须包含**与代码一致的数学化描述过程**。至少覆盖以下内容中实际存在的部分：

1. **对象与集合定义**：输入对象、节点/样本/任务集合、索引、坐标、特征向量、邻接/依赖关系。
2. **核心变换或模型**：矩阵变换、投影、归一化、配准、聚类、评分模型、约束求解等；须说明参数来源和求解方式。
3. **匹配/筛选/决策公式**：代码中的距离、相似度、权重、阈值、损失、排序、布尔条件必须公式化或形式化。
4. **分支与边界条件**：空集合、无匹配、阈值未达、缺失字段、降级路径、回退逻辑应写清楚。
5. **输出构造**：报告、图层、清单、状态、缓存、文件等输出如何由中间结果生成。
6. **参数表**：代码常量、命令行参数、默认阈值和可调参数须列入关键参数，且与正文公式同名或可追溯。

禁止：只写“采用算法进行匹配/评分/筛选”这类泛化表述；禁止编造与代码不一致的公式；禁止只给一两个装饰性公式而遗漏主要计算链路。若代码实现没有 LaTeX 形式，也可用专利正文可读的“文字公式/算式”表达，但必须完整、可追溯、与代码逻辑一致。
