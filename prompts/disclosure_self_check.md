...
- [ ] **源码/算法类材料**：若输入主要为代码，是否已把核心函数、常量、阈值、分支、评分、筛选和输出构造转写为完整数学化/形式化过程？是否能逐项对应源码逻辑，而非泛泛描述？
- [ ] **查新呈现克制**：正文是否只保留最接近通常做法、缺点和本案区别？是否已删除搜索过程、工具名、降级说明、外部链接堆砌和“搜索结果中间内容”？模板含检索附表时，是否把对比条目放入附表而不是正文流水账？
...
- [ ] **用户模板一致性**：若用户提供 Word/Markdown 模板，最终 `.docx` 是否以用户模板为基底或等效保持模板的章节、表格、字段、标题风格、页眉页脚、字体和顺序？是否避免了把默认 Markdown 转 Word 的风格冒充模板风格？是否检查过模板中的占位字段均已填充或保留为“待填写”？
- [ ] **系统框图与流程图**：均为 fenced mermaid，定稿已用 `mermaid_render.py` 转 PNG，**无** ASCII 文字流程图/框图；`.md` 已交付，`.docx` 已生成或已按提示手动 `md_to_docx.py` 补全
- [ ] **模板图示适配**：若用户模板要求 Word 图片/Visio/可编辑图，Word 中是否插入了图片或模板可接受图示？是否不存在未渲染 mermaid 代码块、LaTeX 源码块或无法阅读的图示占位？
